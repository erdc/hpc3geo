...
for i=1:9
   ...
   sPlot(i,:) = S;
   phiMax = quad(@DREVGMpcHat,pcHat(1),pcHat(length(pcHat)));
   phiPlot(i,1) = 0;
   for j=2:length(pcHat)-1
        phiPlot(i,j) = phiPlot(i,j-1) + quad(@DREVGMpcHat,pcHat(j-1),pcHat(j));
   end
   phiPlot(i,length(pcHat)) = phiMax;
   phiPlot(i,:) = phiPlot(i,:) ./ phiMax;
   fPlot(i,:) = -krwVGMpcHat(pcHat);
   dPlot(i,:) = DREVGMpcHat(pcHat);
end
figure(1)
